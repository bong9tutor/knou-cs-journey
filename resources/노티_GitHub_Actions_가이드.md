...
- [ ] 노션 개인 액세스 토큰(PAT) 발급 (2장) - DB 공유 단계 불필요
...
> **PAT vs 연결(Internal connection) 차이**: PAT는 "내 계정 권한"을 그대로 쓰는 토큰이라 **DB를 따로 공유할 필요가 없다**(내가 볼 수 있는 건 자동으로 접근). 반면 "연결(액세스 토큰)"은 별도 봇이라 트래커 DB를 명시적으로 Connect 해야 한다. 둘 다 스크립트에서 동일하게 동작(`Authorization: Bearer` + Notion API)하지만, 단계가 적은 PAT를 쓴다. OAuth 옵션은 브라우저 로그인이 필요해 헤드리스 실행에 안 맞으니 쓰지 않는다.
...
PAT는 토큰을 만든 사용자(본인)가 접근 가능한 모든 페이지/DB에 자동 접근하므로, **트래커 DB를 별도로 Connect 할 필요가 없다.** 본인이 트래커의 소유자/편집자이기만 하면 바로 읽고 진도율을 갱신할 수 있다.

> (참고) 만약 PAT 대신 "연결(액세스 토큰)"으로 만들었다면, 그때는 트래커 DB에서 **... -> 연결(Connections) -> 만든 연결 선택**으로 공유해야 404가 안 난다.
